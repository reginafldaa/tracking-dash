  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white">Data Jadwal</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item) => {
                const pelatihan = pelatihanList.find((p) => p.id === item.pelatihanId);

                return (
                  <tr key={item.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    {/* TANGGAL */}
                    ...
                    {/* PELATIHAN */}
                    <td className="p-3">{pelatihan?.name || 'Tidak ada'}</td>

                    {/* METODE */}
                    <td className="p-3">{item.metode || '-'}</td>

                    {/* LOKASI */}
                    <td className="p-3 text-center">{item.location ? item.location : '-'}</td>

                    {/* STATUS */}
                    <td className="p-3 text-center">
                      <span className="bg-gray-200 px-2 py-1 rounded text-xs">{item.status || '-'}</span>
                    </td>

                    {/* AKSI */}
                    <td className="p-3 text-center">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleEdit(item)} className="bg-yellow-400 hover:bg-yellow-500 px-3 py-1 rounded text-xs">
                          Edit
                        </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-900p-6 rounded w-96">
            <h3 className="text-lg font-bold mb-4">{isEdit ? 'Edit Jadwal' : 'Tambah Jadwal'}</h3>

            <input type="date" name="date" value={form.date} onChange={handleChange} className="w-full border p-2 mb-2" />

            <input type="text" name="location" placeholder="Lokasi" value={form.location} onChange={handleChange} className="w-full border p-2 mb-2" />

            <select name="pelatihanId" value={form.pelatihanId} onChange={handleChange} className="w-full border p-2 mb-4">
            ...
            <select name="metode" value={form.metode} onChange={handleChange} className="w-full border p-2 mb-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={isEdit ? handleUpdate : handleAdd} className="bg-blue-500 text-white px-3 py-1 rounded">
